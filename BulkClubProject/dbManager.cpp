...
dbManager::dbManager()
{
    ...
    bulkdb.setDatabaseName("HiChew.db");

    if(QFile::exists("HiChew.db"))
    {
        if(!bulkdb.open())
        {
 ...
 * \brief dbManager::AddItem
 * Adds item to records based on input
 * information
 * \param newIem - A struct argument
 */
...
/*! To modify item information
 * \brief dbManager::modifyItem
 * Changes record data fro given item
...
bool dbManager::AddPurchase(const Purchase& newPurchase)
{
    ...
    {
        success = true;
        PurchaseUpdateInventory(newPurchase);   // If Purchase was successful, update the inventory details.
        PurchaseUpdateMemberTotalSpent(newPurchase); // If Purchase was successful, update member's total spent
...
bool dbManager::PurchaseUpdateInventory(const Purchase newPurchase)
{
    QSqlQuery query;
    bool success;
    if(ItemExists(newPurchase.GetItem()))  // If the item exists, update it.
    {
...
bool dbManager::PurchaseUpdateMemberTotalSpent(const Purchase newPurchase)
{
    QSqlQuery query;
    bool success;
    if(MemberExists(newPurchase.GetMemberID()))  // If the member exists, update it.
    {
